...
from mongo_db.models import WBProductDiscount
...
async def reserve_products(limit: int):
    ...
    # резервируем новые
    products = await WBProductDiscount.find(
        {
            "published": False,
            "photos_parsed": False,
...
class PhotoParserService:
    ...
    async def process_product(self, product: WBProductDiscount):
...
async def run_free_photo_parser():
